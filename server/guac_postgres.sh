...
docker run --name guac-postgres -e POSTGRES_PASSWORD="$MARACAS_DB_PASS" -d postgres
sleep 5

echo "Create guacamole db"
docker run -e PGPASSWORD="$MARACAS_DB_PASS" --rm --link guac-postgres:postgres postgres\
       psql -h postgres -U postgres -c "CREATE DATABASE guacamole_db;"

echo "Run SQL init script against the db"
docker run -e PGPASSWORD="$MARACAS_DB_PASS" -v $(pwd):/tmp --rm --link guac-postgres:postgres postgres \
